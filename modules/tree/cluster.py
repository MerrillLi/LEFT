...
from modules.tree.bi_kmeans import BiKMeans
...
class ClusterTree(MetaTree):

    def __init__(self, args):
        super().__init__(args)
        assert self.args.narys == 2, 'ClusterTree only support binary tree'
    ...
    def initialize(self, meta_tcom: MetaTC):
        ...
        for ktype in self.args.ktype:
            ...
            tree_allocator = BiKMeans(ktype_embeds.cpu().numpy())
            leaf_allocation = tree_allocator.cluster()
        ...
        self.leaf_mask = torch.zeros((self.num_nodes, ), dtype=torch.int32, device=self.args.device)
        self.node_mask = torch.zeros((self.num_nodes, ), dtype=torch.int32, device=self.args.device)
        self.leaf_mapp = torch.zeros((self.num_nodes, ), dtype=torch.int32, device=self.args.device) - 1
        ...
        if len(self.args.ktype) == 1:
            ...
            type0_allocation = self.allocation[ktype0].to(self.args.device)
            type1_allocation = self.allocation[ktype1].to(self.args.device)
        ...
        # 6. 设置优化器 #
        ################
        self.meta_tcom = copy.deepcopy(meta_tcom)
        self.setup_optimizer()


    def setup_optimizer(self):
        params = list(self.parameters())# + list(self.meta_tcom.parameters())
        self.optimizer = torch.optim.Adam(params, lr=self.args.tree_lr)
